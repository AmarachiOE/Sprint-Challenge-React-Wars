...
React is a javascipt library. React uses a concept called Virtual DOM to handle dynamic data (complex state) at faster rates than the normal DOM can (such as rendering ever-updating social media feeds like facebook and twitter). The virtual DOM only updates nodes that have changed, and not the entire tree; renders just the changes. This improves performance.


- [x] What does it mean to _think_ in react?

Thinking in react means to: break down a project into smaller components of UI to work on; dividing the UI into a component hierarchy. By thinking in react we use data to visualize and drive our view. 

- [x] Describe state.

State is the data that our components can hold on to and pass around to other components as props; when the state updates, so will our components -- this makes react “reactive”.

- [x] Describe props.

Props are properties that carry information from parent components to children components. Functional components receive props as an object passed in as an argument. Props are not mutable, only readable.
